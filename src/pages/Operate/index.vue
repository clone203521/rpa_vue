      ...
      <el-container>
        <el-main>
          ...
          <el-dialog :title="`当前浏览器 - ${currentBrowserId}`" v-model="dialogVisible" custom-class="custom-dialog">
            <!--            <div>浏览器ID: {{ currentBrowserId }}</div>-->
...
const push_city_address = (city_id) => {
...
const splice_city = (city_id)=>{
  city_values.value.splice(city_id, 1)
}
const splice_city_address = (city_id, address_id) => {
...
const getTagList = async () => {
  ...
  try {
    loading.value = true;
    const response = await $http.post('/get_tagList'); // 使用全局配置的 axios
...
const getOperateList = async () => {
  ...
  try {
    loading.value = true;
    const response = await $http.post('/get_operateList'); // 使用全局配置的 axios
...
const openCenter = async () => {
  try {
    const response = await $local.post('/delete_user'); // 使用全局配置的 axios
    ElMessage({
      message: response.data.msg,
      type: response.data.statu_code,
    })
